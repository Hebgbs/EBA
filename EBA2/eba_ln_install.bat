...
ECHO.
IF NOT EXIST %~dp0\.uninstall GOTO eba_ln_init
IF EXIST %~dp0\.uninstall GOTO eba_modinst
...
GOTO msg_ln
:msg_ln
IF EXIST %~dp0\.uninstall (
  COPY NUL > %~dp0\.uninstall2
  ECHO Removing assets
  ECHO ------------------
  GOTO do_ln
)
IF NOT EXIST ~%dp0\.uninstall (
  ECHO Writing assets
  ECHO -----------------
  GOTO do_ln
)
...
CALL :eba_B
CALL :eba_G
IF NOT EXIST %~dp0\.uninstall COPY NUL > %~dp0\.uninstall
GOTO fin_ln

:eba_A
SET src_eba=%~dp0\Aerials
SET des_eba=%des_ln%\cars\misc
IF EXIST %~dp0\.uninstall2 (
